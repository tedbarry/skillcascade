...
const ADAPTIVE_BANDS = [
  { min: 0, max: 1.0, label: 'Low', color: '#FEE2E2', textColor: '#DC2626' },
  { min: 1.0, max: 1.5, label: 'Mod. Low', color: '#FEF3C7', textColor: '#B45309' },
  { min: 1.5, max: 2.5, label: 'Adequate', color: '#FEF9C3', textColor: '#A16207' },
  { min: 2.5, max: 3.0, label: 'High', color: '#D1FAE5', textColor: '#059669' },
]

function getBarColor(score) {
  if (score >= 2.5) return '#059669'
  if (score >= 1.5) return '#D97706'
  if (score >= 1.0) return '#B45309'
  return '#DC2626'
}
...
export function generateDomainBarChart(scores) {
  ...
  for (const v of [1.0, 1.5, 2.0, 2.5]) {
    ...
    svg += `<line x1="${x}" y1="${topMargin}" x2="${x}" y2="${topMargin + chartH}" stroke="#D6D3D1" stroke-width="0.5" stroke-dasharray="3,3"/>`
  }
  ...
  for (let i = 0; i < scores.length && i < 9; i++) {
    ...
    // Domain label
    svg += `<text x="${leftMargin - 8}" y="${y + rowH / 2 + 4}" text-anchor="end" font-size="11" fill="#1C1917" font-weight="500">${escapeHtml(s.domain)}</text>`

    // Bar background
    svg += `<rect x="${leftMargin}" y="${barY}" width="${chartW}" height="${barH}" rx="3" fill="#F5F5F4" opacity="0.4"/>`
    ...
    svg += `<text x="${labelX}" y="${y + rowH / 2 + 4}" font-size="10" fill="#292524" font-weight="600">${s.score.toFixed(1)}</text>`

    // Coverage (assessed/total)
    svg += `<text x="${W - 8}" y="${y + rowH / 2 + 4}" text-anchor="end" font-size="9" fill="#A8A29E">${s.assessed}/${s.total}</text>`
  }

  // X-axis labels
  for (let v = 0; v <= 3; v++) {
    const x = leftMargin + (v / maxScore) * chartW
    svg += `<text x="${x}" y="${topMargin + chartH + 16}" text-anchor="middle" font-size="9" fill="#57534E">${v}</text>`
  }

  // Border
  svg += `<rect x="${leftMargin}" y="${topMargin}" width="${chartW}" height="${chartH}" fill="none" stroke="#D6D3D1" stroke-width="0.5"/>`
...
export function generateRadarChart(scores) {
  ...
  for (const level of [1, 2, 3]) {
    ...
    ring += 'Z'
    svg += `<path d="${ring}" fill="none" stroke="#E7E5E4" stroke-width="0.5"/>`
    // Level label
    const labelP = polar(0, r)
    svg += `<text x="${labelP.x + 4}" y="${labelP.y - 3}" font-size="8" fill="#A8A29E">${level}</text>`
  }

  // Axis lines
  for (let i = 0; i < axes; i++) {
    const p = polar(i * angleStep, R)
    svg += `<line x1="${cx}" y1="${cy}" x2="${p.x.toFixed(1)}" y2="${p.y.toFixed(1)}" stroke="#E7E5E4" stroke-width="0.5"/>`
  }
  ...
  poly += 'Z'
  svg += `<path d="${poly}" fill="rgba(5,150,105,0.2)" stroke="#059669" stroke-width="2"/>`

  // Data points
  for (const p of dataPoints) {
    svg += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3.5" fill="#059669" stroke="white" stroke-width="1.5"/>`
  }
  ...
  for (let i = 0; i < axes; i++) {
    ...
    svg += `<text x="${p.x.toFixed(1)}" y="${p.y.toFixed(1)}" text-anchor="${anchor}" dominant-baseline="middle" font-size="9" fill="#1C1917" font-weight="500">${escapeHtml(name)}</text>`
  }
...
export function generateMasteryGrid(assessments, frameworkData) {
  ...
  for (const item of legendItems) {
    ...
    svg += `<text x="${legendX + 14}" y="${legendY + 9}" font-size="9" fill="#292524">${item.label}</text>`
    legendX += 90
  }
  ...
  for (const domain of frameworkData) {
    // Domain label
    svg += `<text x="${labelW}" y="${y + saLabelH + cellSize / 2}" text-anchor="end" font-size="10" fill="#1C1917" font-weight="600">${escapeHtml(domain.name)}</text>`

    let x = leftPad
    for (const sa of domain.subAreas) {
      // Sub-area tick mark
      svg += `<line x1="${x}" y1="${y + saLabelH - 2}" x2="${x}" y2="${y + saLabelH + cellSize + 2}" stroke="#D6D3D1" stroke-width="0.5"/>`
...
export function generateScoreSummaryProfile(scores, subAreaScores, compositeScore) {
  ...
  function buildDomainPanel() {
    ...
    // Title
    svg += `<text x="${leftM + chartW / 2}" y="16" text-anchor="middle" font-size="11" fill="#1C1917" font-weight="700">Domain Score Profile</text>`
    svg += `<text x="${leftM + chartW / 2}" y="28" text-anchor="middle" font-size="8" fill="#A8A29E" font-style="italic">Criterion-referenced, 0\u20133 scale</text>`
    ...
    for (let v = 0; v <= 3; v += 0.5) {
      ...
      svg += `<line x1="${leftM}" y1="${y}" x2="${leftM + chartW}" y2="${y}" stroke="#D6D3D1" stroke-width="${isDashed ? 0.3 : 0.5}" ${isDashed ? 'stroke-dasharray="2,2"' : ''}/>`
      if (v % 1 === 0) svg += `<text x="${leftM - 3}" y="${y + 3}" text-anchor="end" font-size="8" fill="#57534E">${v}</text>`
    }

    // Vertical divider after composite
    const divX = leftM + colW
    svg += `<line x1="${divX}" y1="${topM}" x2="${divX}" y2="${topM + chartH}" stroke="#1C1917" stroke-width="0.8"/>`
    ...
    if (points.length > 2) {
      ...
      svg += `<path d="${domainPath}" fill="none" stroke="#1C1917" stroke-width="1.5"/>`
    }

    // Dots
    for (const p of points) {
      const r = p.item.isComposite ? 5 : 3.5
      svg += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${r}" fill="${p.item.isComposite ? '#1C1917' : '#059669'}" stroke="white" stroke-width="1.5"/>`
    }

    // Border
    svg += `<rect x="${leftM}" y="${topM}" width="${chartW}" height="${chartH}" fill="none" stroke="#D6D3D1" stroke-width="0.5"/>`
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      svg += `<text font-size="7.5" fill="#1C1917" font-weight="${items[i].isComposite ? '700' : '500'}">${escapeHtml(label)}</text>`
      svg += `</g>`
    }

    // "Score" row label + values
    const scoreRowY = tableY + 68
    svg += `<text x="${leftM - 3}" y="${scoreRowY}" font-size="7.5" fill="#292524" font-weight="700" text-anchor="end">Score</text>`
    for (let i = 0; i < items.length; i++) {
      const x = leftM + i * colW + colW / 2
      if (items[i].isComposite) svg += `<rect x="${x - 13}" y="${scoreRowY - 9}" width="26" height="13" rx="2" fill="#E7E5E4"/>`
      svg += `<text x="${x}" y="${scoreRowY}" text-anchor="middle" font-size="8.5" fill="#1C1917" font-weight="${items[i].isComposite ? '700' : '600'}">${items[i].score.toFixed(2)}</text>`
    }

    // "Level" row
    const levelRowY = scoreRowY + 14
    svg += `<text x="${leftM - 3}" y="${levelRowY}" font-size="7.5" fill="#292524" font-weight="700" text-anchor="end">Level</text>`
    for (let i = 0; i < items.length; i++) {
      const x = leftM + i * colW + colW / 2
      svg += `<text x="${x}" y="${levelRowY}" text-anchor="middle" font-size="7" fill="#292524">${getAdaptiveBandLabel(items[i].score)}</text>`
    }
  ...
  function buildSubAreaPanel() {
    ...
    // Title
    svg += `<text x="${leftM + chartW / 2}" y="16" text-anchor="middle" font-size="11" fill="#1C1917" font-weight="700">Sub-Area Score Profile</text>`
    svg += `<text x="${leftM + chartW / 2}" y="28" text-anchor="middle" font-size="8" fill="#A8A29E" font-style="italic">Criterion-referenced, 0\u20133 scale</text>`
    ...
    for (let i = 0; i < n; i++) {
      if (subAreaScores[i].domainName !== prevDomain) {
        if (prevDomain !== null) {
          ...
          svg += `<line x1="${sx}" y1="${topM}" x2="${sx}" y2="${topM + chartH}" stroke="#A8A29E" stroke-width="0.8"/>`
        }
    ...
    for (const range of domainRanges) {
      ...
      if (hasPoints) svg += `<path d="${segPath}" fill="none" stroke="#1C1917" stroke-width="1.2"/>`
    }

    // Dots
    for (const p of points) {
      if (p.y !== null) {
        svg += `<circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="3" fill="#059669" stroke="white" stroke-width="1"/>`
      } else {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (label.length > 16) label = label.substring(0, 15) + '\u2026'
      svg += `<g transform="translate(${x + 3},${tableY}) rotate(55)">`
      svg += `<text font-size="6" fill="#1C1917" font-weight="500">${escapeHtml(label)}</text>`
      svg += `</g>`
    }
    ...
    for (const range of domainRanges) {
      ...
      const mid = (x1 + x2) / 2
      svg += `<line x1="${x1}" y1="${bracketY}" x2="${x2}" y2="${bracketY}" stroke="#292524" stroke-width="1"/>`
      let dName = range.domain
      if (dName.length > 14) dName = dName.substring(0, 13) + '\u2026'
      svg += `<text x="${mid}" y="${bracketY + 9}" text-anchor="middle" font-size="6.5" fill="#1C1917" font-weight="700">${escapeHtml(dName)}</text>`
    }

    // Score row
    const scoreRowY = bracketY + 22
    svg += `<text x="${leftM - 3}" y="${scoreRowY}" font-size="7" fill="#292524" font-weight="700" text-anchor="end">Score</text>`
    for (let i = 0; i < n; i++) {
      const x = leftM + i * colW + colW / 2
      const sa = subAreaScores[i]
      svg += `<text x="${x}" y="${scoreRowY}" text-anchor="middle" font-size="7" fill="#1C1917" font-weight="600">${sa.assessed > 0 ? sa.score.toFixed(1) : '\u2013'}</text>`
    }
