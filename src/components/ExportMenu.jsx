...
import { generateCSV, generateJSON } from '../data/exportUtils.js'
...
export default function ExportMenu({ assessments, snapshots, clientName }) {
  ...
  function handlePrint() {
    if (!exportAccess.canUseAssessmentExportMenu) return
  ...
  if (permissionsLoading || !exportAccess.canUseAssessmentExportMenu) {
    return null
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        className="text-sm text-warm-500 hover:text-warm-700 px-3 py-1.5 min-h-[44px] rounded-full hover:bg-warm-100 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-sage-500"
      ...
      {isOpen && (
        <>
          ...
          <div
            className="absolute top-full right-0 mt-2 w-56 bg-white border border-warm-200 rounded-xl shadow-sm z-30 overflow-hidden"
            role="menu"
            aria-label="Export options"
            onKeyDown={(e) => {
              if (e.key === 'Escape') {
          ...
          >
            <div className="text-[10px] uppercase tracking-wider text-warm-500 font-semibold px-4 pt-3 pb-1">
              Export Options
            </div>
              ...
              {snapshots.length > 0 && (
                <label className="flex items-center gap-2 px-4 pb-3 pt-1 text-xs text-warm-500 cursor-pointer">
                  <input
                    ...
                    className="rounded border-warm-300 text-sage-500 focus:ring-sage-500"
                  />
