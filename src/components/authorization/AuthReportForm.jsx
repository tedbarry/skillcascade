...
SOCIALIZATION GOALS: ${socialGoals.join('; ') || 'Not specified'}` },
  ...
  const handleImportFromTree = useCallback(async () => {
    ...
    const newGoals = programs
      ...
      .map(p => ({
        id: p.id,
        skillId: p.library_target_id || p.stg_id,
        domain: mapLearningTreeDomainToAuthGoalDomain(p.domain),
        library_target_id: p.library_target_id || null,
        canonical_domain_slug: p.canonical_domain_slug || null,
        canonical_deficit_slug: p.canonical_deficit_slug || null,
