...
export default function KBCategoryGrid({ categoryCounts }) {
  return (
      ...
      {CATEGORY_ORDER.map(key => {
        ...
        return (
          <Link
            key={key}
            to={`/kb?category=${key}`}
            className="group p-5 rounded-xl border border-warm-200 bg-white hover:border-sage-300 hover:shadow-md transition-all"
          >
            <div className="flex items-start gap-3">
              <div className="text-sage-500 group-hover:text-sage-600 transition-colors shrink-0">
                {CATEGORY_ICONS[key]}
              </div>
              <div className="min-w-0">
                ...
                <span className="inline-block mt-2 text-[10px] text-warm-500 bg-warm-50 px-2 py-0.5 rounded-full">
