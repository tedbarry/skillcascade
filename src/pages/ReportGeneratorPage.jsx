...
const workflowSteps = [
  {
    ...
    detail: 'Pick the folder with the diagnostic/evaluation, adaptive assessment such as Vineland, and any optional intake or related records.',
  },
  {
    ...
    <>
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <div className="rounded-xl border border-white/70 bg-white px-3 py-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-warm-500">Source evidence</p>
          <div className="mt-2 space-y-2">
            {evidenceCategories.length ? evidenceCategories.map((category) => (
              <div key={category.id} className="rounded-lg border border-warm-100 bg-warm-50 px-3 py-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-0.5 text-[11px] font-semibold uppercase tracking-wide text-warm-500">
              ...
              <p className="text-sm leading-6 text-warm-600">Run Check files to see required and recommended evidence.</p>
            )}
          </div>
        </div>
    ...
    <div className="mt-4 rounded-xl border border-blue-200 bg-white p-4">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-4">
        <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-3">
          ...
          {summary.recommendedEvidenceTotal ? (
            <p className="mt-1 text-xs font-semibold text-blue-800">
              Recommended: {summary.recommendedEvidenceFound || 0}/{summary.recommendedEvidenceTotal}
            </p>
          ) : null}
        </div>
        <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-3">
...
export default function ReportGeneratorPage() {
  ...
  async function runLocalDraft() {
    ...
    if (needsEvidenceCheck) {
      setRunState({ loading: false, result: null, error: 'Run Check files first. Required source evidence must be present before creating a report draft. Intake/caregiver history can be added when available but is not required.' })
      return
    }
          ...
          <section className="rounded-2xl border border-warm-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              ...
              {userCanEdit && creditBalance > 0 && needsEvidenceCheck ? (
                <span className="text-xs font-semibold text-amber-700">Run Check files first. Evaluation and adaptive evidence must be present before generation. Intake is optional.</span>
              ) : null}
            </div>
