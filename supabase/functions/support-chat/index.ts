...
function buildSystemPromptWithContext(context: {
  ...
  if (summary?.assessed) {
    ...
    const domainAvgs = summary.domainAverages as Array<{ domain: string; average: number; assessedCount: number; state: string }> | undefined
    ...
    const topStrengths = summary.topStrengths as string[] | undefined
    if (topStrengths?.length) {
      parts.push(`- Top strengths (mastered skills): ${topStrengths.join('; ')}`)
    }

    const cascadeRisks = summary.cascadeRisks as Array<{ title: string; description: string }> | undefined
