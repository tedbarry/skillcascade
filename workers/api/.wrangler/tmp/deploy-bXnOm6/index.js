...
var support_chat_default = app14;
...
var stripe_checkout_default = app15;
...
var stripe_portal_default = app16;
...
var stripe_webhook_default = app17;
...
app18.get("/health", (c) => c.json({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
app18.route("/webhooks/stripe", stripe_webhook_default);
app18.post("/api/contact", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const body = await c.req.json();
...
app18.use("/api/*", authMiddleware);
app18.route("/api/profiles", profiles_default);
app18.route("/api/clients", clients_default);
app18.route("/api/assessments", assessments_default);
app18.route("/api/programs", programs_default);
app18.route("/api/sessions", sessions_default);
app18.route("/api/goals", goals_default);
app18.route("/api/organizations", organizations_default);
app18.route("/api/subscriptions", subscriptions_default);
app18.route("/api/reports", reports_default);
app18.route("/api/audit", audit_default);
app18.route("/api/ai-chats", ai_chats_default);
app18.route("/api/data", generic_default);
app18.route("/api/ai-proxy", ai_proxy_default);
app18.route("/api/support-chat", support_chat_default);
app18.route("/api/stripe-checkout", stripe_checkout_default);
app18.route("/api/stripe-portal", stripe_portal_default);
app18.post("/api/rpc/ensure_user_org", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const result = await dbQuery(
    c.env,
...
app18.post("/api/rpc/create_client_for_user", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const body = await c.req.json();
  const result = await dbQuery(
...
app18.post("/api/rpc/delete_client_for_user", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const body = await c.req.json();
  const result = await dbQuery(
...
app18.notFound((c) => c.json({ error: "Not found" }, 404));
app18.onError((err, c) => {
  console.error("API error:", err.message, err.stack);
  return c.json({ error: "Internal server error" }, 500);
});
var src_default = app18;
export {
