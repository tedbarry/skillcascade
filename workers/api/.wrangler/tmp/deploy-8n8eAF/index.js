...
function canEditSessionNoteRecord(profile3, note = {}) {
  ...
  if (SESSION_NOTE_EDITOR_ROLES.has(roleSlug))
    return true;
  return roleSlug === "rbt" && note.staff_id === profile3.id && ["draft", "completed"].includes(note.status || "draft");
}
...
var SESSION_NOTE_TRANSITION_FIELDS = {
  completed: ["status", "completed_by", "completed_at"],
  reviewed: ["status", "reviewed_by", "reviewed_at"],
  approved: ["status", "approved_by", "approved_at"]
};
...
async function validateSessionNoteUpdate(env2, profile3, note, updates = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "status")) {
    ...
    if (nextStatus === "completed") {
      updates.completed_by = profile3.id;
      updates.completed_at = updates.completed_at || timestamp;
    } else if (nextStatus === "reviewed") {
      updates.reviewed_by = profile3.id;
      updates.reviewed_at = updates.reviewed_at || timestamp;
    } else if (nextStatus === "approved") {
      updates.approved_by = profile3.id;
      updates.approved_at = updates.approved_at || timestamp;
    }
    return { allowed: true };
  }
  if (!keys.every((key) => SESSION_NOTE_EDITABLE_FIELDS.has(key))) {
...
async function enforceAccess(env2, profile3, table3, operation, body, filters, rawData = null) {
  ...
  if (table3 === "session_notes") {
    ...
    const existingNote = await loadSingleRow(
      ...
      "session_notes",
      id,
      "id, org_id, client_id, staff_id, status, session_id"
    );
