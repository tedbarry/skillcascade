...
var app12 = new Hono2();
var TABLE_CONFIG = {
  // Client-scoped tables (require canAccessClient via client_id)
  ...
  // access controlled via program_id which cascades
  // User-scoped tables
  ...
  // Org-scoped tables (also have dedicated routes but queried via api.from())
  ...
  // filtered by client_id in frontend; user scope was showing all reports by creator regardless of client
  reports: { scope: "open" },
  ...
  // Session notes, client files, client contacts
...
async function enforceAccess(env2, profile3, table3, operation, body, filters) {
  ...
  if (scope === "org") {
    ...
    if (operation === "insert") {
      ...
      if (!filters)
        filters = {};
      filters[orgCol] = profile3.org_id;
    }
    return { allowed: true, filters };
  }
  if (scope === "client") {
  ...
  if (scope === "lookup") {
    ...
    const parent = await query(
      env2,
      `SELECT ${config2.lookupClientColumn} FROM ${config2.lookupTable} WHERE id = $1`,
      [lookupId]
    );
  ...
  try {
    ...
    switch (operation) {
      ...
      case "update":
        ...
        break;
      case "upsert":
        result = await handleUpsert(c.env, profile3, table3, mutData, on_conflict || onConflict, returning);
        break;
      case "delete":
...
function buildWhereClause(filters, startIdx = 1) {
  ...
  return { clause: conditions.length > 0 ? `WHERE ${conditions.join(" AND ")}` : "", values, nextIdx: idx };
}
...
async function handleSelect(env2, table3, columns, filters, order, limit, offset) {
  ...
  if (order && order.column) {
    ...
    sql += ` ORDER BY ${order.column} ${dir3}`;
  }
  if (limit)
...
async function handleUpsert(env2, profile3, table3, data, onConflict2, returning) {
  const rows = Array.isArray(data) ? data : [data];
  if (rows.length === 0)
    return [];
  const cols = Object.keys(rows[0]);
  ...
  const conflictCols = onConflict2 || "id";
  const updateCols = cols.filter((c) => !conflictCols.split(",").map((s) => s.trim()).includes(c)).map((c) => `${c} = EXCLUDED.${c}`).join(", ");
  let sql = `INSERT INTO ${table3} (${cols.join(", ")}) VALUES ${rowPlaceholders.join(", ")}`;
...
var support_chat_default = app15;
...
var stripe_checkout_default = app16;
...
var stripe_portal_default = app17;
...
var stripe_webhook_default = app18;
...
app19.get("/health", (c) => c.json({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
app19.route("/webhooks/stripe", stripe_webhook_default);
app19.post("/api/contact", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const body = await c.req.json();
...
app19.use("/api/*", authMiddleware);
app19.route("/api/profiles", profiles_default);
app19.route("/api/clients", clients_default);
app19.route("/api/assessments", assessments_default);
app19.route("/api/programs", programs_default);
app19.route("/api/sessions", sessions_default);
app19.route("/api/goals", goals_default);
app19.route("/api/organizations", organizations_default);
app19.route("/api/subscriptions", subscriptions_default);
app19.route("/api/reports", reports_default);
app19.route("/api/audit", audit_default);
app19.route("/api/ai-chats", ai_chats_default);
app19.route("/api/data", generic_default);
app19.route("/api/client-files", client_files_default);
app19.route("/api/ai-proxy", ai_proxy_default);
app19.route("/api/support-chat", support_chat_default);
app19.route("/api/stripe-checkout", stripe_checkout_default);
app19.route("/api/stripe-portal", stripe_portal_default);
app19.post("/api/rpc/ensure_user_org", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const result = await dbQuery(
    c.env,
...
app19.post("/api/rpc/create_client_for_user", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const body = await c.req.json();
  const result = await dbQuery(
...
app19.post("/api/rpc/delete_client_for_user", async (c) => {
  const { query: dbQuery } = await Promise.resolve().then(() => (init_db(), db_exports));
  const userId = c.get("userId");
  const body = await c.req.json();
  const result = await dbQuery(
...
app19.notFound((c) => c.json({ error: "Not found" }, 404));
app19.onError((err, c) => {
  console.error("API error:", err.message, err.stack);
  return c.json({ error: "Internal server error" }, 500);
});
var src_default = app19;
export {
