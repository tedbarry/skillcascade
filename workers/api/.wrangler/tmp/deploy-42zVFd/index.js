...
var SESSION_NOTE_VIRTUAL_FIELDS = /* @__PURE__ */ new Set(["workflow_reason"]);
...
async function writeSessionNoteStatusAudit(env2, profile3, beforeNote, afterNote, workflowReason = "") {
...
async function enforceAccess(env2, profile3, table3, operation, body, filters, rawData = null) {
  ...
  if (table3 === "session_notes") {
    ...
    if (operation === "update") {
      const validation = await validateSessionNoteUpdate(env2, profile3, existingNote, body);
      if (!validation.allowed)
        return validation;
