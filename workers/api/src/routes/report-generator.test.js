...
function validRunProof(overrides = {}) {
  return {
    ...
    helperVersion: '0.1.0',
    packageVersion: 'release-20260618-pdf-status-v13',
...
describe('report-generator route contract', () => {
  ...
  it('returns protected helper package status for authorized users', async () => {
    ...
    expect(payload.currentVersion).toBe('release-20260618-pdf-status-v13')
    expect(payload.minimumVersion).toBe('release-20260618-pdf-status-v13')
  ...
  it('returns the latest helper manifest metadata for authorized users', async () => {
    ...
    expect(response.status).toBe(200)
    expect(payload.ok).toBe(true)
    expect(payload.version).toBe('release-20260618-pdf-status-v13')
    expect(payload.minimumVersion).toBe('release-20260618-pdf-status-v13')
    expect(payload.installerName).toBe('Install-ReportGeneratorHelper.exe')
    expect(payload.downloadPath).toBe('/api/report-generator/helper/download')
    expect(bucket.head).toHaveBeenCalled()
  ...
  it('downloads the protected helper package as a zip', async () => {
    ...
    expect(response.headers.get('x-skillcascade-report-helper-version')).toBe('release-20260618-pdf-status-v13')
    expect(response.headers.get('x-skillcascade-report-helper-minimum-version')).toBe('release-20260618-pdf-status-v13')
  ...
  it('rejects credit consumption from an outdated helper run proof', async () => {
    ...
    expect(payload.requiredVersion).toBe('release-20260618-pdf-status-v13')
    expect(db.query.mock.calls.some((call) => String(call[1]).includes('INSERT INTO report_generator_credit_ledger'))).toBe(false)
  })
  ...
  it('upserts a safe local helper install claim for the signed-in user', async () => {
    ...
    expect(payload.data.claim.requiredHelperVersion).toBe('release-20260618-pdf-status-v13')
  ...
  it('lists safe install claims for the signed-in user', async () => {
      ...
      if (String(sql).includes('SELECT id, org_id, user_id, install_fingerprint')) {
        return {
          rows: [{
            ...
            required_helper_version: 'release-20260618-pdf-status-v13',
    ...
    expect(payload.data.claims[0].requiredHelperVersion).toBe('release-20260618-pdf-status-v13')
  })
})
