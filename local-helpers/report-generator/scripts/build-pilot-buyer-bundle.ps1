param(
  [string]$Version = "pilot-$(Get-Date -Format 'yyyyMMdd-HHmm')",
...
$BundleName = "SkillCascadeReportGeneratorPilot-$Version"
...
Set-Content -LiteralPath (Join-Path $BundleRoot 'pilot-manifest.json') -Encoding ASCII -Value ($manifest | ConvertTo-Json -Depth 4)

$readme = @"
SkillCascade Report Generator Pilot Bundle
Version: $Version

What this is:
- A local-first helper package for the SkillCascade Report Generator workflow pack.
- The helper reads local source folders and Word templates on the buyer workstation.
- SkillCascade coordinates the workflow, access, onboarding checklist, and install claim.
...
Set-Content -LiteralPath (Join-Path $BundleRoot 'README-FIRST.txt') -Encoding ASCII -Value $readme
  ...
  [pscustomobject]@{
    ...
    manifestPath = Join-Path $BundleRoot 'pilot-manifest.json'
